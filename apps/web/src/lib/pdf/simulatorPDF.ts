...
function buildSummaryData(data: SimulationPDFData, roundingUnit: number): PDFSummaryData {
  ...
  const roundedInstallment = roundForDisplay(data.result.installmentAmount, roundingUnit);
  const totalPayment = data.result.schedule.reduce((sum, item) => {
    return sum + roundForDisplay(item.payment, roundingUnit);
  }, 0);
...
export function generateSimulatorPDF(data: SimulationPDFData, roundingUnit: number): void {
  ...
  const tableData = data.result.schedule.map((item) => [
    ...
    `$${roundForDisplay(item.payment, roundingUnit).toLocaleString()}`,
  ]);
